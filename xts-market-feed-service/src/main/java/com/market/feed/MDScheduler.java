...
public class MDScheduler implements XTSAPIMarketdataEvents {
    ...
    @Scheduled(cron = "0 20 08 * * MON-FRI")
    ...
    @Scheduled(cron = "0 31 08 * * MON-FRI")
...
//            System.out.println("TouchLineUnSubscribeResponse : " + unsubscribeResponse.getResult());
    ...
    public void subscribeInstrumentData(Result result, ExpiryDateResponse expiryDateResponse, String series ,String subScribeField) {
        ...
        try {
            ...
            double ltpValue = result.getBhavcopy().getClose();

            if (ltpValue == 0) {
                try {
                    if (result.getName().equalsIgnoreCase("NIFTY 50")) {
                        MarketData marketData = redisService.findMarketData("MD_26000");
                        ltpValue = marketData.getClose();
                    } else if (result.getName().equalsIgnoreCase("NIFTY BANK")) {
                        MarketData marketData = redisService.findMarketData("MD_26001");
                        ltpValue = marketData.getClose();
                    } else if (result.getName().equalsIgnoreCase("NIFTY FIN SERVICE")) {
                        MarketData marketData = redisService.findMarketData("MD_26034");
                        ltpValue = marketData.getClose();
                    } else if (result.getName().equalsIgnoreCase("BANKEX")) {
                        MarketData marketData = redisService.findMarketData("MD_26118");
                        ltpValue = marketData.getClose();
                    } else if (result.getName().equalsIgnoreCase("SENSEX")) {
                        MarketData marketData = redisService.findMarketData("MD_26065");
                        ltpValue = marketData.getClose();
                    }
                } catch (Exception e) {
                    logger.error("fetching ltpValue failed {}", e.getMessage());
                    ltpValue = 0;
                }
            }

            final double LTP = ltpValue;
...
//            boolean deleted = redisService.deleteKey("Jwt");
